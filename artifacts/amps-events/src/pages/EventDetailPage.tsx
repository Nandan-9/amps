...
import { formatDate, getCategoryColor, getStatusColor } from '@/utils/eventUtils';
...
export default function EventDetailPage() {
  ...
  const initialEvent = events.find(e => e.id === id);
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (count > 0) {
            setEvent(prev => {
              if (!prev) return prev;
              const updatedSeatsRemaining = prev.seats ? Math.max(0, (prev.seatsRemaining ?? prev.seats) - count) : 0;
              const updatedParticipantCount = (prev.participantCount ?? 0) + count;
              return {
                ...prev,
                seatsRemaining: updatedSeatsRemaining,
  ...
  if (!event) {
    return (
      <div className="min-h-screen bg-[#050508] flex flex-col items-center justify-center text-center p-4">
        ...
        <Link href="/" className="px-6 py-3 glass rounded-full hover:bg-white/10 transition-colors">
          Return to Events
        </Link>
      </div>
    );
      ...
      <nav className="fixed top-0 inset-x-0 z-50 glass border-b border-white/10 bg-black/20 backdrop-blur-xl">
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/" className="p-2 rounded-full glass hover:bg-white/10 transition-colors group">
            ...
            {(isLive || isUpcoming) && event.startDate && (
              <div className="mt-8 flex items-center gap-6">
                ...
                {isLive && event.streamingLink && (
                  <a href={event.streamingLink} target="_blank" rel="noreferrer" className="px-6 py-3 rounded-full bg-red-500 hover:bg-red-600 text-white font-bold flex items-center gap-2 transition-colors animate-pulse-dot">
                ...
                <div className="relative pl-6 sm:pl-8 border-l-2 border-white/10 space-y-12">
                  {agenda.map((item, i) => (
                    <div key={i} className="relative">
                      <div className="absolute -left-[35px] sm:-left-[43px] top-1 w-5 h-5 rounded-full bg-[#050508] border-4 border-primary z-10" />
                  ...
                  {speakers.map(speaker => (
                    <div key={speaker.id} className="glass p-6 rounded-3xl border-white/5 hover:bg-white/[0.08] transition-colors group">
                      <div className="flex gap-4">
                        <img src={speaker.photo} alt={speaker.name} className="w-20 h-20 rounded-full object-cover ring-2 ring-primary/20 group-hover:ring-primary transition-all" />
                        <div>
            ...
            <section>
              <AnimatedSection>
                ...
                <div className="grid md:grid-cols-3 gap-6 mt-8 items-end">
                  {/* Reorder to 2nd, 1st, 3rd for podium look on desktop */}
                  {[
                ...
                <div className="glass rounded-3xl overflow-hidden border-white/10 p-2">
                  <div className="relative pt-[56.25%] w-full rounded-2xl overflow-hidden bg-black">
                    <Player 
                      ...
                      controls
                      light // Shows thumbnail before playing
                    />
                  </div>
                </div>
              </AnimatedSection>
                ...
                <div className="glass rounded-3xl overflow-hidden flex flex-col md:flex-row border-white/10">
                  <div className="p-8 md:w-1/3 flex flex-col justify-center bg-white/[0.02]">
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:text-cyan-400 transition-colors w-fit"
                    >
              ...
              <div className="bg-[#0A0E1A]/80 backdrop-blur-xl rounded-[22px] p-6 md:p-8">
                ...
                {(isUpcoming || isLive) && registrationLink && registrationLink !== '#' ? (
                  <a 
                    ...
                    onClick={(e) => seatsRemaining === 0 && seats > 0 && e.preventDefault()}
                  >
                ...
                <div className="mt-8 pt-6 border-t border-white/10">
                  ...
                  <div className="flex justify-center gap-3">
                    ...
                    <a href={`https://wa.me/?text=${encodeURIComponent(`${event.title} - ${window.location.href}`)}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full glass flex items-center justify-center hover:bg-[#25D366]/20 transition-colors group">
                      <div className="w-4 h-4 text-white/60 group-hover:text-[#25D366] font-bold">W</div>
                    </a>
                    <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(`Check out ${event.title} by AMPS!`)}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full glass flex items-center justify-center hover:bg-white/20 transition-colors group">
                      <div className="w-4 h-4 text-white/60 group-hover:text-white font-bold">X</div>
                    </a>
                    <a href={`https://linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full glass flex items-center justify-center hover:bg-[#0A66C2]/20 transition-colors group">
